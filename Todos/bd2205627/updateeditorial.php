<?php
include 'conexion.php';
?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" type="text/css" href="editorial.css">
    <title>Verificar datos</title>
</head>
<body>
<?php
    try{
        $sql1="SELECT * FROM editorial WHERE Id=".$_REQUEST['cod'].";";
        $resultado = $base->prepare($sql1);
          $resultado->execute(array());
          while ($consulta = $resultado->fetch(PDO::FETCH_ASSOC)) {
          ?>
          <form action="updateeditorial2.php?new=<?php echo $_REQUEST['cod']; ?>" name="frmact"
          method="POST">
          <table  class="actualizar">
              <tr>
                  <td colspan="2" class="titulo">Actualizando editorial - <?php
                  echo $consulta['Nombre'] ?> - </td>
              </tr>
              <tr>
                  <td>Nombre:</td>
                  <td><input type="text" name="txt2" requied value="<?php echo $consulta['
                  Nombre'] ?>"></td>
              </tr>
              <tr>
              <td>Pais:</td>
                  <td><input type="text" name="txt2" requied value="<?php echo $consulta['
                  Pais'] ?>"></td>
              </tr>
              <tr>
              <td>Año:</td>
                  <td><input type="text" name="txt2" requied value="<?php echo $consulta['
                  year_fabricacion'] ?>"></td>
              </tr>
              <tr>
                  <td colspan="2"><input type="submit" name="btna" value="Actualizar..." class="boton"></td>
              </tr>
          </table>
          </form>
</body>
</html>